/*
 * Contract tests for plugin-scm-github, incl. the parent → child delegation:
 * when scm-github is registered, plugin-scm's renderFeatures/renderDetailsKey
...
describe('plugin-scm-github manifest', () => {
  it('exposes a valid tool-level manifest', () => {
    expect(def.id).toBe('scm-github')
  ...
  it('merges i18n on install', () => {
    ...
    def.install()
    expect(i18n.t('service:scm:github:repository')).toBeTypeOf('string')
    expect(i18n.t('service:scm:github:repository')).not.toBe('service:scm:github:repository')
  })
  ...
  it('renderFeatures returns a home-link button when params are set', () => {
    def.install()
    const vnodes = def.feature('renderFeatures', { parameters: {"service:scm:github:user":"acme","service:scm:github:repository":"app"} })
  ...
  it('renderDetailsKey returns the resource chip when present', () => {
    def.install()
    expect(def.feature('renderDetailsKey', { parameters: { 'service:scm:github:repository': 'x' } })).toBeTruthy()
    expect(def.feature('renderDetailsKey', { parameters: {} })).toBeNull()
  })
})

describe('plugin-scm → plugin-scm-github delegation', () => {
  beforeEach(() => {
    parentDef.install({ router: { addRoute() {} } })
    def.install()
    pluginRegistry.register('scm-github', def)
  })
  afterEach(() => { pluginRegistry.remove('scm-github') })

  it('parent renderDetailsKey resolves to this tool for a matching node', () => {
    const out = parentDef.feature('renderDetailsKey', {
      node: { id: 'service:scm:github:1' },
      parameters: { 'service:scm:github:repository': 'x' },
    })
